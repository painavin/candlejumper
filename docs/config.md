...
| Key | Description | Default |
|---|---|---|
| `scrollSpeed` | Speed in **bars per second** (trading days per second), not pixels — resolution-independent, and the auto-bounce cadence derives from it. Range 0.5–10. This is the speed a run *starts* at: the player steps it up and down mid-run with `←`/`→` ([controls.md](./controls.md#scroll-speed-while-playing)), which is run-scoped and never written back here. Excluded from the run fingerprint | 2 *(provisional)* |
...
| `preloadBars` | Bars fed to indicators and stops **before play starts**, so their values exist on the first bar the player trades. `0` is off; `'auto'` takes the largest `warmupBars` across the active indicators and everything the configured stops pull in ([indicators.md](./indicators.md#preloading-warm-up-bars)). Preloaded bars are consumed, not played: they never reach the trading tick and never score, but they *are* drawn as history to the left of the cursor, so the run opens on a full window with the indicators already across it. They come off the *front* of whatever was loaded, including off the front of a chosen `data.dateRange`, so preload shortens the run. **Included in the run fingerprint when non-zero**, since starting 200 bars in is a different challenge; a run with preload off hashes exactly as it did before the key existed | `0` |
